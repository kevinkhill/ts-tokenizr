...
class Tokenizr {
     ...
     */
    stateRule(state, pattern, action, name = "unknown") {
        return this.pushRule({
            state: util_1.postProcessState(state),
            pattern: util_1.postProcessPattern(pattern),
            action,
            name
        });
    }
     ...
     */
    rule(pattern, action, name = "unknown") {
        return this.pushRule({
            state: { state: "*", tags: [] },
            pattern: util_1.postProcessPattern(pattern),
            action,
    ...
    _tokenize() {
        ...
        while (continued) {
            ...
            for (let i = 0; i < this._rules.length; i++) {
                if (this.config.debug) {
                    this._log(`  RULE: state(s): <${State_1.State.stringify(this._rules[i].state)}>, pattern: ${this._rules[i].pattern.source}`);
                }
